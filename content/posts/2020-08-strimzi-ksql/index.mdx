---
title: Use KSQL on kuberentes with Strizi
slug: ksql-strimzi-k8s
draft: true
date: 2020-08-23
description: Setup KSQL on Kubernetes with Strimzi Kafka Operator
---

[Strimzi](https://strimzi.io/) is a super simple Kubernetes Operators that allow you to run a kafka
cluster on Kuberetes in minutes. In this post, we will see how to install
...
To install the strimzi operator you simply need to run the following command:
...
this will define a kafka cluster with 1 zookeeper node and 1 kafka node, you
...
you should see the new pods in your k8s cluster
...
now, you can access the kafka using the `kafka-cluster-kafka-bootstrap` ip on port 9091 inside the cluster.
...
To install a ksql node, we also need a schema registry node that is required by ksql.

We will use helm (version 3 in my case) to install both schema registry and ksql. We will use the [helm charts](https://github.com/confluentinc/cp-helm-charts) provided by
...
The schema registry is simple to install, we need only to provide the `kafkaBootstrap` configuration and leave
...
Last step is to install the ksql server. You need to access the directory `charts/cp-schema-registry` to install the chart.
...
```bash
$ kubectl run tmp-ksql-cli --rm -i --tty --image confluentinc/cp-ksql-cli:5.2.1 http://ksql-server-cp-ksql-server:8088
