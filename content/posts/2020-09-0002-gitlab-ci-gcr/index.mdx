...
Recently I stated working on **GCP Cloud Run** and I need to
...
Build docker image with GitLab CI is easy, but, first of all, you need a project on Gitlab CI and where to deploy your project.
...
To test locally, you can run it with `npm run start`, and open the browser on the url you
get on the console.
...
To do so, you need to create a file named `.gitlab-ci.yml` that contains the instructions
...
To do so, in our GPC project where we wont to deploy the docker images,
...
a JSON file con your computer. This file will be used to login
...
in order to use it on the deploy job. To do so, we will save it
...
The pipeline is complete, the complete `.gitlab-ci.yaml` file is
